...
class Plugin(object):
    ...
    def add_copy_spec(self, copyspecs, sizelimit=None, tailit=True, pred=None):
        ...
        if pred is not None and not pred:
            self._log_info("skipped copy spec '%s' due to predicate" %
                           copyspecs)
            return
    ...
    def _add_cmd_output(self, cmd, suggest_filename=None,
        ...
        if pred is not None and pred:
            ...
            self._log_info("skipped cmd output '%s' due to predicate" % cmd)
    ...
    def get_cmd_output_now(self, exe, suggest_filename=None,
        ...
        pred = self.get_predicate(pred=pred, cmd=True)
        if pred is not None and not pred:
            self._log_info("skipped cmd output '%s' due to predicate" % exe)
            return None
